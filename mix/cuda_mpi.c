...
void cuda_call(int buf_size, int local_buf_size, char *dBuf, char *pattern, int approx_factor, int *ret);
...
int main( int argc, char ** argv )
{
  ...
  char *dBuf;
  dBuf = cuda_malloc( local_buf, local_buf_size );
  ...
  for ( i = 0 ; i < nb_patterns ; i++ )
  {
      ...
      cuda_call(buf_size, local_buf_size, dBuf, pattern[i], approx_factor, &ret);
